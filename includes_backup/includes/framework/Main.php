...
namespace NTWP\Framework;

// PSR-4重构完成，直接使用完整的命名空间类名
...
class Main {
	 ...
	 * @since    1.0.5
	 * @access   protected
	 * @var      \NTWP\Framework\Loader    $loader    维护和注册插件的所有钩子。
	 */
	protected \NTWP\Framework\Loader $loader;
	 ...
	 * @since    1.0.6
	 * @access   protected
	 * @var      \\NTWP\Admin\Admin $admin    后台区域处理器实例。
	 */
	protected $admin;
	 ...
	 * @since    1.1.0
	 * @access   protected
	 * @var      \NTWP\Handlers\Webhook    $webhook    Webhook 处理器实例。
	 */
	protected \NTWP\Handlers\Webhook $webhook;
	 ...
	 * Services层 - 业务逻辑服务：
	 * - API. API接口服务。
	 * - Content_Converter. 内容转换服务。
	 * - Database_Renderer. 数据库渲染服务。
	 * - Image_Processor. 图片处理服务。
	 * - Metadata_Extractor. 元数据提取服务。
	 * - Sync_Manager. 同步管理服务。
	 *
	 * Handlers层 - 协调器服务：
	...
	private function instantiate_objects() {
		...
		// 实例化Webhook处理器
		$this->webhook = new \NTWP\Handlers\Webhook(
			$this->notion_pages
		);
	}
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_get_system_info', $this->admin, 'handle_get_system_info' );
		...
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_get_performance_metrics', $this->admin, 'handle_get_performance_metrics' );
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_clear_cache', $this->admin, 'handle_clear_cache' );
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_optimize_database', $this->admin, 'handle_optimize_database' );

		// 日志管理相关AJAX钩子
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_get_logs', $this->admin, 'handle_get_logs' );
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_clear_all_logs', $this->admin, 'handle_clear_all_logs' );
		$this->loader->add_action( 'wp_ajax_notion_to_wordpress_get_log_files_list', $this->admin, 'handle_get_log_files_list' );
	...
	private function define_public_hooks() {
		...
		// 初始化特色图像支持
		$this->loader->add_action( 'init', '\NTWP\Handlers\Integrator', 'init_featured_image_support' );
	}
