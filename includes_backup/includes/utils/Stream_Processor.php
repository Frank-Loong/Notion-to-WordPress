...
class Stream_Processor {
     ...
     */
    public static function process_large_dataset(array $blocks, \NTWP\Services\API $notion_api, string $state_id = null): string {
        if (empty($blocks)) {
            return '';
        }
     ...
     * @return string HTML内容
     */
    private static function convert_single_block_lightweight(array $block, \NTWP\Services\API $notion_api): string {
        $block_type = $block['type'] ?? 'unknown';
            ...
            switch ($block_type) {
                ...
                default:
                    ...
                    if (class_exists('\\NTWP\\Services\\Content_Converter')) {
                        return \NTWP\Services\Content_Converter::convert_single_block_optimized($block, $notion_api, []);
                    }
                    return '';
            }
        } catch (Exception $e) {
