...
class Database_Helper {
    ...
    public static function create_performance_indexes(): array {
        ...
        if (class_exists('Notion_Logger')) {
            \NTWP\Core\Logger::info_log('开始创建数据库性能优化索引', 'Database Index Optimizer');
        }
        ...
        if (empty($suggestions)) {
            ...
            if (class_exists('Notion_Logger')) {
                \NTWP\Core\Logger::info_log('所有必要的索引已存在', 'Database Index Optimizer');
            }
            return $results;
        }
        ...
        try {
            ...
            if (self::needs_meta_key_index()) {
                ...
                if ($index_result['success']) {
                    ...
                    if (class_exists('Notion_Logger')) {
                        \NTWP\Core\Logger::info_log('成功创建meta_key索引', 'Database Index Optimizer');
                    }
                } else {
            ...
            if (self::needs_composite_index()) {
                ...
                if ($composite_result['success']) {
                    ...
                    if (class_exists('Notion_Logger')) {
                        \NTWP\Core\Logger::info_log('成功创建复合索引', 'Database Index Optimizer');
                    }
                } else {
    ...
    public static function remove_performance_indexes(): array {
        ...
        try {
            ...
            if (\NTWP\Utils\Database_Index_Manager::drop_index('postmeta', 'idx_notion_meta_key')) {
            ...
            // 删除复合索引 - 使用正确的MySQL语法
            if (\NTWP\Utils\Database_Index_Manager::drop_index('postmeta', 'idx_notion_meta_key_value')) {
            ...
            if (class_exists('\\NTWP\\Core\\Logger') && $results['success']) {
                \NTWP\Core\Logger::info_log('成功删除性能优化索引', 'Database Index Optimizer');
            }
