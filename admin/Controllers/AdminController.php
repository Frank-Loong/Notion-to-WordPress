...
namespace NTWP\Admin\Controllers;
...
use NTWP\Infrastructure\Memory\MemoryManager;
use NTWP\Core\Performance\PerformanceMonitor;
use NTWP\Infrastructure\Database\DatabaseHelper;
use NTWP\Infrastructure\Database\IndexManager;
 ...
 * 后台管理控制器类
 *
 * 负责插件后台设置页面的功能，包括表单处理、选项保存、AJAX请求处理等。
 * 采用现代化的控制器模式，提供清晰的API接口和统一的错误处理机制。
 *
 * @since      2.0.0-beta.1
...
class AdminController {
     ...
     * @var      ImportHandler
     */
    private ImportHandler $notion_pages;
     ...
     * @param NotionApi $notion_api Notion API实例
     * @param ImportHandler $notion_pages Notion导入处理器实例
     */
    public function __construct(string $plugin_name, string $version, NotionApi $notion_api, ImportHandler $notion_pages) {
    ...
    public function display_plugin_setup_page() {
        require_once Helper::plugin_path('admin/Views/AdminDisplay.php');
    }
    ...
    private function parse_settings(array $options): array {
        ...
        if (class_exists('NTWP\\Utils\\Config_Simplifier')) {
            ...
            if (!isset($options['config_migrated'])) {
                $options = ConfigSimplifier::migrate_legacy_config($options);
            }

            // 应用简化配置到详细配置
            $options = ConfigSimplifier::apply_simplified_config($options);
        }
    ...
    private function parse_react_settings(array $options): array {
        ...
        if (!$settings_data) {
            \NTWP\Core\Foundation\Logger::error_log('无法获取React设置数据', 'Parse React Settings');
            return $options;
        }
    ...
    public function handle_get_settings() {
        try {
            ...
            \NTWP\Core\Foundation\Logger::debug_log('获取设置AJAX验证成功', 'Get Settings');
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Get Settings']);
                \NTWP\Core\Foundation\Logger::error_log(
    ...
    public function handle_save_settings_ajax() {
        try {
            ...
            if (isset($_POST['settings'])) {
                ...
                $validation_result = $this->validate_ajax_request('notion_to_wordpress_nonce');
                \NTWP\Core\Foundation\Logger::debug_log('使用React格式nonce验证', 'Save Settings');
            } else {
                // 传统表单数据使用原有的nonce验证
                $validation_result = $this->validate_ajax_request('notion_to_wordpress_options_update', 'notion_to_wordpress_options_nonce');
                \NTWP\Core\Foundation\Logger::debug_log('使用传统表单nonce验证', 'Save Settings');
            }
            ...
            if (isset($_POST['settings'])) {
                \NTWP\Core\Foundation\Logger::debug_log('检测到React格式数据，使用parse_react_settings处理', 'Save Settings');
                $options = $this->parse_react_settings($current_options);
            } else {
                \NTWP\Core\Foundation\Logger::debug_log('检测到传统表单数据，使用parse_settings处理', 'Save Settings');
                $options = $this->parse_settings($current_options);
            }

            // 使用统一验证框架验证配置参数
            if (class_exists('\\NTWP\\Core\\Security')) {
                $validation = \NTWP\Core\Foundation\Security::validate_plugin_options($options);
    ...
    public function handle_test_connection() {
        try {
            ...
            // 使用传入的Key和ID进行测试
            $temp_api = new NotionApi($api_key);
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Test Connection']);
    ...
    public function handle_manual_import() {
        try {
            // 添加调试日志
            \NTWP\Core\Foundation\Logger::debug_log('手动导入开始执行', 'Manual Import');
            ...
            \NTWP\Core\Foundation\Logger::debug_log('手动导入AJAX验证成功', 'Manual Import');

            // 获取任务ID并初始化进度跟踪
        $task_id = sanitize_text_field($_POST['task_id'] ?? '');
        $progress_tracker = null;

        if (!empty($task_id) && class_exists('NTWP\\Core\\Performance\\ProgressTracker')) {
            try {
                // 使用新的静态方法创建任务，传递前端的task_id以确保一致性
                $created_task_id = \NTWP\Core\Foundation\Performance\ProgressTracker::create_task(
                    'manual_sync',
                    0, // 初始时不知道总数
                    [
                        'sync_type' => 'manual',
                ...
                // 使用传统方法创建Progress_Tracker实例（用于ImportHandler）
                $progress_tracker = new \NTWP\Core\Foundation\Performance\ProgressTracker();
        ...
        try {
            ...
            $notion_api = new NotionApi( $api_key );

            error_log('Notion to WordPress: 创建导入协调器实例，Database ID: ' . $database_id);
            $notion_pages = new ImportHandler( $notion_api, $database_id, $field_mapping );
            ...
            if (!empty($task_id) && class_exists('NTWP\\Core\\Performance\\ProgressTracker')) {
                try {
                    \NTWP\Core\Foundation\Performance\ProgressTracker::complete_task($task_id, [
            ...
            // 外层异常处理（包括验证异常）
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Manual Import']);
    ...
    public function handle_get_stats() {
        try {
            ...
            \NTWP\Core\Foundation\Logger::debug_log('获取统计信息AJAX验证成功', 'Get Stats');
            // 获取导入的文章数量
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Get Stats']);
    ...
    public function handle_clear_logs() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Clear Logs']);
    ...
    public function handle_view_log() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'View Log']);
    ...
    public function handle_refresh_verification_token() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Refresh Verification Token']);
    ...
    public function handle_test_debug() {
        try {
            ...
            \NTWP\Core\Foundation\Logger::debug_log('Test debug AJAX验证成功', 'AJAX Handler');
    ...
    public function handle_refresh_performance_stats() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Refresh Performance Stats']);
    ...
    public function handle_reset_performance_stats() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Reset Performance Stats']);
    ...
    public function handle_create_database_indexes() {
        try {
            ...
            // 调用数据库助手创建索引
            $result = DatabaseHelper::create_performance_indexes();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Create Database Indexes']);
    ...
    public function handle_get_index_status() {
        try {
            ...
            // 获取索引状态
            $status = DatabaseHelper::get_index_status();

            // 获取专用Notion索引优化建议
            $notion_suggestions = DatabaseHelper::get_notion_specific_optimization_suggestions();
            $general_suggestions = DatabaseHelper::get_optimization_suggestions();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Get Index Status']);
    ...
    public function handle_remove_database_indexes() {
        try {
            ...
            // 调用数据库助手删除索引
            $result = DatabaseHelper::remove_performance_indexes();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Remove Database Indexes']);
    ...
    public function handle_optimize_all_indexes() {
        try {
            ...
            // 调用一键优化方法
            $result = IndexManager::optimize_all_notion_indexes();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Optimize All Indexes']);
    ...
    public function handle_get_queue_status() {
        try {
            ...
            if (class_exists('NTWP\\Core\\Task\\ModernAsyncEngine')) {
                $system_status = ModernAsyncEngine::getStatus();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Get Queue Status']);
    ...
    public function handle_cancel_queue_task() {
        try {
            ...
            // 使用现代异步引擎
            if (class_exists('NTWP\\Core\\Task\\ModernAsyncEngine')) {
                $result = ModernAsyncEngine::cancel($task_id);
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Cancel Queue Task']);
    ...
    public function handle_cleanup_queue() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Cleanup Queue']);
    ...
    public function handle_get_async_status() {
        try {
            ...
            if (class_exists('NTWP\\Core\\Async_Processor')) {
                $async_status = \NTWP\Core\Foundation\Async_Processor::get_async_status();
    ...
    public function handle_control_async_operation() {
        try {
            ...
            if (class_exists('NTWP\\Core\\Async_Processor')) {
                ...
                switch ($action) {
                    case 'pause':
                        $result = \NTWP\Core\Foundation\Async_Processor::pause_async_operation();
                        $message = $result ? '异步操作已暂停' : '暂停失败，可能没有运行中的操作';
                        break;

                    case 'resume':
                        $result = \NTWP\Core\Foundation\Async_Processor::resume_async_operation();
                        $message = $result ? '异步操作已恢复' : '恢复失败，可能没有暂停的操作';
                        break;

                    case 'stop':
                        $result = \NTWP\Core\Foundation\Async_Processor::stop_async_operation();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Async Operation Control']);
    ...
    public function handle_cancel_sync() {
        try {
            ...
            // 取消任务
            if (class_exists('NTWP\\Core\\Task\\ModernAsyncEngine')) {
                $result = \NTWP\Core\Foundation\ModernAsyncEngine::cancel($task_id);
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Cancel Sync']);
    ...
    public function handle_retry_failed() {
        try {
            ...
            // 重试失败项
            if (class_exists('NTWP\\Core\\Task\\ModernAsyncEngine')) {
                $result = \NTWP\Core\Foundation\ModernAsyncEngine::retryFailed($task_id);
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Retry Failed']);
    ...
    public function handle_sse_progress() {
        try {
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'SSE Progress']);
                \NTWP\Core\Foundation\Logger::error_log(
    ...
    public function handle_database_indexes_request() {
        try {
            // 记录调试信息
            if (class_exists('\\NTWP\\Core\\Logger')) {
                \NTWP\Core\Foundation\Logger::info_log('Database Indexes Request: 开始处理数据库索引请求', 'Database Indexes Request');
            }
            ...
            if (!$validation_result['is_valid']) {
                if (class_exists('\\NTWP\\Core\\Logger')) {
                    \NTWP\Core\Foundation\Logger::error_log('Database Indexes Request: 验证失败 - ' . $validation_result['error_message'], 'Database Indexes Request');
                }
                wp_send_json_error(['message' => $validation_result['error_message']], $validation_result['http_code']);
                return;
            }
            ...
            if (empty($action)) {
                if (class_exists('\\NTWP\\Core\\Logger')) {
                    \NTWP\Core\Foundation\Logger::error_log('Database Indexes Request: 缺少索引操作参数', 'Database Indexes Request');
                }
            ...
            if (class_exists('\\NTWP\\Core\\Logger')) {
                \NTWP\Core\Foundation\Logger::info_log('Database Indexes Request: 执行操作 - ' . $action, 'Database Indexes Request');
            }
            switch ($action) {
                case 'create_all':
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Database Indexes Request']);
    ...
    private function handle_create_database_indexes_optimized() {
        if (class_exists('\\NTWP\\Infrastructure\\Database\\IndexOptimizer')) {
            $start_time = microtime(true);
            $result = \NTWP\Infrastructure\Database\IndexOptimizer::create_all_indexes();
    ...
    private function handle_get_database_indexes_status() {
        try {
        if (class_exists('\\NTWP\\Infrastructure\\Database\\IndexOptimizer')) {
            $status = \NTWP\Infrastructure\Database\IndexOptimizer::get_indexes_status();
            $recommendations = \NTWP\Infrastructure\Database\IndexOptimizer::get_query_optimization_recommendations();
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Get Database Indexes Status']);
                wp_send_json_error(['message' => __('获取索引状态失败: ', 'notion-to-wordpress') . $error->get_error_message()]);
            } else {
                wp_send_json_error(['message' => __('获取索引状态失败: ', 'notion-to-wordpress') . $e->getMessage()]);
            }
    ...
    private function handle_drop_database_index() {
        ...
        if (class_exists('\\NTWP\\Infrastructure\\Database\\IndexOptimizer')) {
            $result = \NTWP\Infrastructure\Database\IndexOptimizer::drop_index($index_name);
    ...
    private function handle_maintain_database_indexes() {
        if (class_exists('\\NTWP\\Infrastructure\\Database\\IndexOptimizer')) {
            $start_time = microtime(true);
            $result = \NTWP\Infrastructure\Database\IndexOptimizer::maintain_indexes();
            $execution_time = microtime(true) - $start_time;
    ...
    public function handle_analyze_query_performance() {
        try {
            ...
            } elseif (class_exists('\\NTWP\\Infrastructure\\Database\\IndexOptimizer')) {
                // 向后兼容：使用适配器
                $analysis = \NTWP\Infrastructure\Database\IndexOptimizer::analyze_query_performance($query);
            ...
            // 使用统一错误处理
            if (class_exists('NTWP\\Core\\Foundation\\ErrorHandler')) {
                $error = \NTWP\Core\Foundation\ErrorHandler::exception_to_wp_error($e, ['context' => 'Analyze Query Performance']);
