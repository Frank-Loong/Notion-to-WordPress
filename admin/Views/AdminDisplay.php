...
namespace NTWP\Admin\Views;

/**
 * 后台管理页面视图
 *
 * 负责渲染插件的主设置页面，包括 API 配置、数据库同步、字段映射等。
 * 采用现代化的视图模式，提供清晰的界面结构和用户体验。
 *
 * @since      2.0.0-beta.1
...
$debug_level           = $options['debug_level'] ?? \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_ERROR;
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $webhook_token = $options['webhook_token'] ?? \NTWP\Infrastructure\Helper::generate_token(32);
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $performance_levels = class_exists('NTWP\\Utils\\Config_Simplifier')
                                            ? \NTWP\Infrastructure\ConfigSimplifier::get_available_performance_levels()
                                            : [
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        $field_templates = class_exists('NTWP\\Utils\\Config_Simplifier')
                                            ? \NTWP\Infrastructure\ConfigSimplifier::get_available_field_templates()
                                            : [
                        ...
                        <div class="notion-wp-performance-dashboard">
                                ...
                                <div class="notion-wp-performance-card">
                                    ...
                                    <div class="notion-wp-performance-config">
                                        ...
                                        if (class_exists('NTWP\\Core\\Performance_Monitor')) {
                                            $config = \NTWP\Core\Foundation\Performance\PerformanceMonitor::get_performance_config();
                                        } else {
                                            // 获取插件选项作为备用配置
                                            $options = get_option('notion_to_wordpress_options', []);
                                            $config = [
                            ...
                            <div class="notion-wp-performance-card">
                                ...
                                if (class_exists('NTWP\\Core\\Queue_Manager')) {
                                    try {
                                        $queue_status = \NTWP\Core\Foundation\Queue_Manager::get_queue_status();
                                    } catch (Exception $e) {
                                        // 使用默认状态
                                    }
                                }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select id="debug_level" name="debug_level">
                                            <option value="<?php echo \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_NONE; ?>" <?php selected($debug_level, \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_NONE); ?>><?php esc_html_e('无日志', 'notion-to-wordpress'); ?></option>
                                            <option value="<?php echo \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_ERROR; ?>" <?php selected($debug_level, \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_ERROR); ?>><?php esc_html_e('仅错误', 'notion-to-wordpress'); ?></option>
                                            <option value="<?php echo \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_WARNING; ?>" <?php selected($debug_level, \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_WARNING); ?>><?php esc_html_e('警告和错误', 'notion-to-wordpress'); ?></option>
                                            <option value="<?php echo \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_INFO; ?>" <?php selected($debug_level, \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_INFO); ?>><?php esc_html_e('信息、警告和错误', 'notion-to-wordpress'); ?></option>
                                            <option value="<?php echo \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_DEBUG; ?>" <?php selected($debug_level, \NTWP\Core\Foundation\Logger::DEBUG_LEVEL_DEBUG); ?>><?php esc_html_e('所有日志 (调试)', 'notion-to-wordpress'); ?></option>
                                        </select>
                                    ...
                                    <td>
                                        <div id="log-viewer-container">
                                            <select id="log-file-selector">
                                                <?php foreach (\NTWP\Core\Foundation\Logger::get_log_files() as $file): ?>
