...
class DatabaseHelper {
    ...
    public static function ultra_batch_get_sync_data(array $notion_ids): array {
        ...
        if (class_exists('PerformanceMonitor')) {
            ...
            Notion_PerformanceMonitor::record_custom_metric('ultra_batch_sync_time', $processing_time);
            Notion_PerformanceMonitor::record_custom_metric('ultra_batch_sync_count', count($notion_ids));
            Notion_PerformanceMonitor::record_custom_metric('ultra_batch_memory_usage', $memory_used);
        }
    ...
    public static function batch_update_sync_status(array $sync_updates): bool {
        ...
        if (class_exists('PerformanceMonitor')) {
            ...
            Notion_PerformanceMonitor::record_custom_metric('batch_update_time', $processing_time);
            Notion_PerformanceMonitor::record_custom_metric('batch_update_success_count', $success_count);
            Notion_PerformanceMonitor::record_custom_metric('batch_update_error_count', $error_count);
        }
    ...
    public static function create_performance_indexes(): array {
        ...
        if (class_exists('Notion_Logger')) {
            \NTWP\Core\Foundation\Logger::info_log('开始创建数据库性能优化索引', 'Database Index Optimizer');
        }
        ...
        if (empty($suggestions)) {
            ...
            if (class_exists('Notion_Logger')) {
                \NTWP\Core\Foundation\Logger::info_log('所有必要的索引已存在', 'Database Index Optimizer');
            }
            return $results;
        }
        ...
        try {
            ...
            if (self::needs_meta_key_index()) {
                ...
                if ($index_result['success']) {
                    ...
                    if (class_exists('Notion_Logger')) {
                        \NTWP\Core\Foundation\Logger::info_log('成功创建meta_key索引', 'Database Index Optimizer');
                    }
                } else {
            ...
            if (self::needs_composite_index()) {
                ...
                if ($composite_result['success']) {
                    ...
                    if (class_exists('Notion_Logger')) {
                        \NTWP\Core\Foundation\Logger::info_log('成功创建复合索引', 'Database Index Optimizer');
                    }
                } else {
        ...
        if (class_exists('PerformanceMonitor')) {
            $processing_time = microtime(true) - $start_time;
            Notion_PerformanceMonitor::record_custom_metric('index_creation_time', $processing_time);
            Notion_PerformanceMonitor::record_custom_metric('indexes_created_count', count($results['created_indexes']));
            Notion_PerformanceMonitor::record_custom_metric('performance_improvement_percent', $results['performance_improvement']);
        }
    ...
    public static function remove_performance_indexes(): array {
        ...
        try {
            ...
            if (\NTWP\Infrastructure\Database\IndexManager::drop_index('postmeta', 'idx_notion_meta_key')) {
            ...
            // 删除复合索引 - 使用正确的MySQL语法
            if (\NTWP\Infrastructure\Database\IndexManager::drop_index('postmeta', 'idx_notion_meta_key_value')) {
            ...
            if (class_exists('\\NTWP\\Core\\Logger') && $results['success']) {
                \NTWP\Core\Foundation\Logger::info_log('成功删除性能优化索引', 'Database Index Optimizer');
            }
