...
class DatabaseRenderer {
     ...
     * @return array 预处理数据
     */
    public static function batch_preprocess_child_databases_optimized(array $child_databases, \NTWP\Services\Api\NotionApi $notion_api): array {
     ...
     * @return string HTML内容
     */
    public static function render_child_database_with_timeout(string $database_id, string $database_title, \NTWP\Services\Api\NotionApi $notion_api, int $timeout_seconds = 10): string {
        $start_time = microtime(true);
        ...
        switch ($prop_type) {
            ...
            case 'rich_text':
                if (!empty($prop_value['rich_text'])) {
                    return \NTWP\Services\Content\TextProcessor::extract_rich_text_complete($prop_value['rich_text']);
                }
                return '';
     ...
     * @return string HTML内容
     */
    public static function render_database_preview_records(string $database_id, array $database_info, \NTWP\Services\Api\NotionApi $notion_api): string {
        try {
            // 获取数据库中的记录
     ...
     */
    public static function batch_process_child_databases(array $database_blocks, \NTWP\Services\Api\NotionApi $notion_api): array {
        ...
        switch ($type) {
            ...
            case 'rich_text':
                if (!empty($property['rich_text'])) {
                    $text = \NTWP\Services\Content\TextProcessor::extract_rich_text_complete($property['rich_text']);
                    // 移除HTML标签以获取纯文本长度
        ...
        foreach ($properties as $name => $property) {
            if (isset($property['type']) && $property['type'] === 'title') {
                if (!empty($property['title'])) {
                    return \NTWP\Services\Content\TextProcessor::extract_rich_text_complete($property['title']);
                }
            }
        }
     ...
     * @return string HTML内容
     */
    public static function render_child_database(string $database_id, string $database_title, \NTWP\Services\Api\NotionApi $notion_api): string {
        try {
            \NTWP\Core\Foundation\Logger::debug_log(
