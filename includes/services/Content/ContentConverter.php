...
class ContentConverter {
    ...
    public static function enable_data_preloader(array $context = []): void {
        if (!class_exists('\\NTWP\\Infrastructure\\Database\\DatabaseHelper')) {
            return;
        }
        ...
        // 预加载相关数据
        if (!empty($context)) {
            \NTWP\Infrastructure\Database\DatabaseHelper::preload_related_data($context);
        }
     ...
     * @param \NTWP\Services\Api\NotionApi $notion_api Notion API 实例
     * @param string $state_id 状态管理器ID，用于图片处理状态隔离
     * @return string HTML 内容
     */
    public static function convert_blocks_to_html(array $blocks, \NTWP\Services\Api\NotionApi $notion_api, string $state_id = null): string {
        // 检查是否启用性能模式
        ...
        if (!empty($database_blocks)) {
            ...
            try {
                // 使用优化后的批量处理
                $database_data = \NTWP\Services\Content\DatabaseRenderer::batch_preprocess_child_databases_optimized(
                    $database_blocks,
                    $notion_api
                );
     ...
     */
    private static function _convert_child_blocks(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        $child_blocks = [];
     ...
     * 转换段落块
     */
    private static function _convert_block_paragraph(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换一级标题块
     */
    private static function _convert_block_heading_1(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换二级标题块
     */
    private static function _convert_block_heading_2(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换三级标题块
     */
    private static function _convert_block_heading_3(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换无序列表项块
     */
    private static function _convert_block_bulleted_list_item(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换有序列表项块
     */
    private static function _convert_block_numbered_list_item(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换待办事项块
     */
    private static function _convert_block_to_do(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换折叠块
     */
    private static function _convert_block_toggle(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换子页面块
     */
    private static function _convert_block_child_page(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换分割线块
     */
    private static function _convert_block_divider(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        return '<hr>';
    }
     ...
     * 转换引用块
     */
    private static function _convert_block_quote(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换代码块
     */
    private static function _convert_block_code(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换图片块
     */
    private static function _convert_block_image(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        ...
        if (\NTWP\Services\Content\ImageProcessor::is_async_image_mode_enabled()) {
            // 异步模式：收集图片信息并返回占位符
            return \NTWP\Services\Content\ImageProcessor::collect_image_for_download($url, $caption);
        } else {
            // 同步模式：直接生成图片HTML
     ...
     * 转换标注块
     */
    private static function _convert_block_callout(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换书签块
     */
    private static function _convert_block_bookmark(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换数学公式块
     */
    private static function _convert_block_equation(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        $expression = $block['equation']['expression'] ?? '';

        // 使用统一的公式处理方法
        return \NTWP\Services\Content\TextProcessor::process_math_expression($expression, 'block');
    }

    /**
     * 转换表格块
     */
    private static function _convert_block_table(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        // 获取所有行（优先复用 children）
        if (isset($block['children']) && is_array($block['children'])) {
            $rows = $block['children'];
        } elseif ($block['has_children'] ?? false) {
            try {
                ...
                \NTWP\Core\Foundation\Logger::error_log("获取表格行失败: " . $e->getMessage(), 'Table Block');
                return '<!-- 表格加载失败 -->';
            }
        } else {
            return '<!-- 空表格 -->';
     ...
     * 转换表格行块
     */
    private static function _convert_block_table_row(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换嵌入块
     */
    private static function _convert_block_embed(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换视频块
     */
    private static function _convert_block_video(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换列列表块
     */
    private static function _convert_block_column_list(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        // 列表容器
     ...
     * 转换列块
     */
    private static function _convert_block_column(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换文件块
     */
    private static function _convert_block_file(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换PDF块
     */
    private static function _convert_block_pdf(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        $pdf_data = $block['pdf'] ?? [];
     ...
     * 转换同步块
     */
    private static function _convert_block_synced_block(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        // 直接渲染其子块
     ...
     * 转换页面链接块
     */
    private static function _convert_block_link_to_page(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * 转换子数据库块（标准版本）
     */
    private static function _convert_block_child_database(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
        ...
        try {
            ...
            if (isset(self::$preprocessed_database_data[$database_id])) {
                ...
                return \NTWP\Services\Content\DatabaseRenderer::render_from_preprocessed_data(
                    $database_id,
                    $database_title,
                    $preprocessed_data
                );
            ...
            // 回退到标准渲染（带超时保护）
            return \NTWP\Services\Content\DatabaseRenderer::render_child_database_with_timeout(
                $database_id,
                $database_title,
                $notion_api,
                10 // 10秒超时
     ...
     * 转换子数据库块（使用预处理数据）
     */
    private static function _convert_block_child_database_with_data(array $block, \NTWP\Services\Api\NotionApi $notion_api, array $database_data): string {
        $database_title = $block['child_database']['title'] ?? '未命名数据库';
        $database_id = $block['id'];
        ...
        if (isset($database_data[$database_id])) {
            ...
            if (!empty($data) && is_array($data) && isset($data['info']) && isset($data['records'])) {
                ...
                $rendered_content = \NTWP\Services\Content\DatabaseRenderer::render_database_preview_records_with_data(
                    $database_id,
                    $data['info'],
                    $data['records']
                );
    ...
    private static function extract_rich_text($rich_text): string {
        ...
        foreach ($rich_text as $text) {
            ...
            if ( isset( $text['type'] ) && $text['type'] === 'equation' ) {
                ...
                $content = \NTWP\Services\Content\TextProcessor::process_math_expression($expr_raw, 'inline');
            } else {
                // 对纯文本内容进行转义
            ...
            if (!empty($href)) {
                // 检测是否为 Notion 锚点链接
                if (\NTWP\Services\Content\TextProcessor::is_notion_anchor_link($href)) {
                    // 转换为本地锚点链接，不添加 target="_blank"
                    $local_href = \NTWP\Services\Content\TextProcessor::convert_notion_anchor_to_local($href);
                    $content = '<a href="' . esc_attr($local_href) . '">' . $content . '</a>';
                } elseif (\NTWP\Services\Content\TextProcessor::is_notion_page_link($href)) {
                    // 转换为 WordPress 永久链接，不添加 target="_blank"
                    $wordpress_href = \NTWP\Services\Content\TextProcessor::convert_notion_page_to_wordpress($href);
     ...
     */
    public static function convert_block_to_html(array $block, \NTWP\Services\Api\NotionApi $notion_api): string {
     ...
     * @return string HTML 内容
     */
    private static function convert_blocks_to_html_optimized(array $blocks, \NTWP\Services\Api\NotionApi $notion_api, string $state_id = null): string {
        if (empty($blocks)) {
            return '';
        }
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('content_conversion');
        }

        // 简化的策略选择：只基于块数量
        $block_count = count($blocks);

        if (class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager')) {
            \NTWP\Infrastructure\Memory\MemoryManager::monitor_memory_usage('Content Conversion');
        }

        if (class_exists('\\NTWP\\Core\\Logger')) {
            \NTWP\Core\Foundation\Logger::debug_log(
                sprintf('内容转换开始: %d个块', $block_count),
                'Content Converter'
            );
        }

        // 简化的策略选择：<100个块用normal模式，≥100个块用chunked模式
        if ($block_count >= 100) {
            return self::convert_blocks_chunked($blocks, $notion_api, $state_id);
        }

        // 使用算法优化器预处理文本内容（降低阈值，让小数据集也能受益）
        if (class_exists('\\NTWP\\Core\\Performance\\AlgorithmOptimizer') && class_exists('\\NTWP\\Services\\Content\\TextProcessor') && count($blocks) > 3) {
            $blocks = \NTWP\Services\Content\TextProcessor::optimized_batch_text_processing($blocks);
        ...
        } elseif (class_exists('\\NTWP\\Core\\Logger') && count($blocks) <= 3) {
            \NTWP\Core\Foundation\Logger::debug_log(
        ...
        foreach ($blocks as $block_index => $block) {
            ...
            if (class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager')) {
                // 根据系统性能调整检查频率
                $stats = \NTWP\Infrastructure\Memory\MemoryManager::get_adaptive_stats();
            ...
            if ($block_index % $check_frequency === 0 && class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager')) {
                if (\NTWP\Infrastructure\Memory\MemoryManager::is_memory_warning()) {
                    \NTWP\Infrastructure\Memory\MemoryManager::force_garbage_collection();
                }
            }
        ...
        // 处理异步图片（如果启用了异步模式）
        if (class_exists('\\NTWP\\Services\\Content\\ImageProcessor') && \NTWP\Services\Content\ImageProcessor::is_async_image_mode_enabled()) {
            // 使用并行图片处理器处理收集到的图片
            $result = \NTWP\Services\Content\ImageProcessor::process_async_images($result);
        ...
        // 结束性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('content_conversion');
        }
     ...
     * @param \NTWP\Services\Api\NotionApi $notion_api API实例
     * @param array $database_data 预处理的数据库数据
     * @return string HTML内容
     */
    private static function convert_single_block_optimized(array $block, \NTWP\Services\Api\NotionApi $notion_api, array $database_data = []): string {
        $block_type = $block['type'];
        $converter_method = '_convert_block_' . $block_type;
        ...
        if (!method_exists(self::class, $converter_method)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                \NTWP\Core\Foundation\Logger::debug_log("未支持的块类型: {$block_type}", 'Block Conversion');
            }
            return '<!-- 未支持的块类型: ' . esc_html($block_type) . ' -->';
        }
     ...
     * @return array 子内容数据
     */
    private static function batch_get_children_content(array $blocks, \NTWP\Services\Api\NotionApi $notion_api): array {
        $children_content = [];
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            ...
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_children_fetch_time', $processing_time);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_children_count', count($blocks_with_children));
        }
     ...
     * @return string HTML 内容
     */
    private static function convert_blocks_chunked(array $blocks, \NTWP\Services\Api\NotionApi $notion_api, string $state_id = null): string {
        if (empty($blocks)) {
            return '';
        }

        $block_count = count($blocks);
        $chunk_size = 50; // 每次处理50个块

        if (class_exists('\\NTWP\\Core\\Logger')) {
            \NTWP\Core\Foundation\Logger::debug_log(
                sprintf('分块处理模式: %d个块，每块%d个', $block_count, $chunk_size),
                'Content Converter Chunked'
            );
        }
        ...
        // 分块处理
        for ($i = 0; $i < $block_count; $i += $chunk_size) {
            $chunk = array_slice($blocks, $i, $chunk_size);
            ...
            foreach ($chunk as $block) {
                ...
                // 转换块
                try {
                    $block_html = self::convert_single_block_optimized($block, $notion_api, []);
                    if (!empty($block_html)) {
                        $block_html = self::wrap_block_with_id($block_html, $block['id'], $block_type);
                        $html_parts[] = $block_html;
                    }
                    ...
                    $html_parts[] = '<!-- 块转换失败: ' . esc_html($block_type) . ' -->';
                }
            }

            // 内存管理
            if (class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager') && ($i + $chunk_size) % 100 === 0) {
                \NTWP\Infrastructure\Memory\MemoryManager::force_garbage_collection();
            }
        }
        ...
        if (class_exists('\\NTWP\\Core\\Logger')) {
            \NTWP\Core\Foundation\Logger::debug_log(
                sprintf('分块处理完成: 处理了%d/%d个块', $processed_count, $block_count),
                'Content Converter Chunked'
            );
        }
