...
class ImageProcessor {
    ...
    public static function process_images_parallel(array $image_urls, int $max_concurrent = self::DEFAULT_MAX_CONCURRENT): array {
        ...
        foreach ($chunks as $chunk_index => $chunk) {
            ...
            if (class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager')) {
                \NTWP\Infrastructure\Memory\MemoryManager::monitor_memory_usage('Parallel Image Processing');
            }
    ...
    private static function process_image_chunk_parallel(array $image_urls): array {
        ...
        // 使用现有的并发网络管理器
        if (class_exists('\\NTWP\\Utils\\Concurrent_Network_Manager')) {
            $manager = new \NTWP\Infrastructure\Concurrent_Network_Manager(count($image_urls));
