...
class TextProcessor {
    ...
    public static function extract_rich_text_complete(array $rich_text): string {
        ...
        if (class_exists('NTWP\Core\Performance\PerformanceMonitor')) {
            ...
            \NTWP\Core\Performance\PerformanceMonitor::record_custom_metric('rich_text_processing_time', $processing_time);
            \NTWP\Core\Performance\PerformanceMonitor::record_custom_metric('rich_text_items_processed', count($rich_text));
        }
    ...
    public static function optimized_batch_text_processing(array $text_blocks): array {
        ...
        if (class_exists('NTWP\Core\Algorithm_Optimizer')) {
            $optimized_texts = \NTWP\Core\Foundation\Performance\AlgorithmOptimizer::optimize_string_operations($texts);
    ...
    public static function get_performance_stats(): array {
        ...
        if (class_exists('NTWP\Core\Algorithm_Optimizer')) {
            $cache_stats = \NTWP\Core\Foundation\Performance\AlgorithmOptimizer::get_cache_stats();
    ...
    public static function batch_extract_rich_text(array $rich_text_array): array {
        ...
        if (class_exists('NTWP\Core\Performance\PerformanceMonitor')) {
            $processing_time = microtime(true) - $start_time;
            \NTWP\Core\Performance\PerformanceMonitor::record_custom_metric('batch_rich_text_processing_time', $processing_time);
            \NTWP\Core\Performance\PerformanceMonitor::record_custom_metric('batch_rich_text_count', count($rich_text_array));
        }
    ...
    public static function clear_link_cache(): void {
        ...
        if (class_exists('NTWP\Core\Foundation\Logger')) {
            \NTWP\Core\Foundation\Logger::debug_log('链接缓存已清理', 'Text Processor');
        }
    }
