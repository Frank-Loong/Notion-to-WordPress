...
class Incremental_Detector {
    ...
    public static function hybrid_incremental_detection(array $api_filtered_pages, string $database_id): array {
        ...
        foreach ($api_filtered_pages as $page) {
            ...
            // 获取对应的WordPress文章ID
            $post_mapping = \NTWP\Utils\Database_Helper::batch_get_posts_by_notion_ids([$notion_id]);
            $post_id = $post_mapping[$notion_id] ?? 0;
        ...
        if (class_exists('\\NTWP\\Core\\Performance_Monitor')) {
            \NTWP\Core\Performance_Monitor::record_custom_metric('hybrid_detection_time', $processing_time);
            \NTWP\Core\Performance_Monitor::record_custom_metric('hybrid_api_trust_count', $api_trust_count);
            \NTWP\Core\Performance_Monitor::record_custom_metric('hybrid_local_verify_count', $local_verify_count);
        }
