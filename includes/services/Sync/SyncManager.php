...
class SyncManager {
    ...
    public static function filter_pages_for_incremental_sync(array $pages): array {
        ...
        // 批量获取同步时间
        $sync_times = \NTWP\Infrastructure\Database\DatabaseHelper::batch_get_sync_times($notion_ids);

        // 使用增量内容检测器进行精确变化检测
        if (class_exists('\\NTWP\\Services\\Sync\\IncrementalDetector')) {
            $post_mapping = \NTWP\Infrastructure\Database\DatabaseHelper::batch_get_posts_by_notion_ids($notion_ids);
            $changes_map = \NTWP\Services\Sync\IncrementalDetector::batch_detect_changes($pages, $post_mapping);
        ...
        foreach ($pages as $page) {
            ...
            if ((class_exists('\\NTWP\\Services\\Sync\\IncrementalDetector')) && isset($post_mapping[$page_id]) && $post_mapping[$page_id] > 0) {
                // 使用增量检测器进行精确检测（优先级最高）
                if (\NTWP\Services\Sync\IncrementalDetector::should_skip_sync($page, $post_mapping[$page_id])) {
                    $should_sync = false;
     ...
     * @return array 同步统计
     */
    public static function super_batch_sync(array $pages, \NTWP\Services\Api\NotionApi $notion_api): array {
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('super_batch_sync');
        }
        ...
        // 第二步：批量获取现有映射（一次查询）
        $existing_mapping = \NTWP\Infrastructure\Database\DatabaseHelper::batch_get_posts_by_notion_ids($notion_ids);
        ...
        // 结束性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('super_batch_sync');
        }
     ...
     * @return array 创建统计
     */
    private static function super_batch_create_posts(array $pages, \NTWP\Services\Api\NotionApi $notion_api): array {
        ...
        // 监控内存使用
        \NTWP\Infrastructure\Memory\MemoryManager::monitor_memory_usage('Batch Create Posts');

        $options = get_option('notion_to_wordpress_options', []);

        // 使用自适应批量大小调整器（整合到Memory_Manager）
        if (class_exists('\\NTWP\\Infrastructure\\Memory\\MemoryManager')) {
            $batch_size = \NTWP\Infrastructure\Memory\MemoryManager::get_optimal_batch_size('database_operations');
        ...
        foreach ($chunks as $chunk_index => $chunk) {
            ...
            // 批量插入
            $result = \NTWP\Handlers\Integrator::batch_insert_posts($posts_data);
     ...
     * @return array 更新统计
     */
    private static function super_batch_update_posts(array $pages, \NTWP\Services\Api\NotionApi $notion_api): array {
        $stats = ['success' => 0, 'errors' => 0];
        ...
        // 监控内存使用
        \NTWP\Infrastructure\Memory\MemoryManager::monitor_memory_usage('Batch Update Posts');
        ...
        foreach ($chunks as $chunk_index => $chunk) {
            ...
            if (!empty($meta_updates)) {
                \NTWP\Handlers\Integrator::batch_update_post_meta($meta_updates);
            }
    ...
    public static function ultra_optimized_get_sync_data(array $notion_ids): array {
        ...
        // 开始性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('ultra_optimized_sync_data');
        }
        ...
        if (count($notion_ids) >= 1000) {
            // 大数据集使用临时表+JOIN优化
            $sync_data = \NTWP\Infrastructure\Database\DatabaseHelper::ultra_batch_get_sync_data($notion_ids);
            ...
            // 中小数据集使用标准批量查询
            $sync_data = \NTWP\Infrastructure\Database\DatabaseHelper::batch_get_sync_times($notion_ids);
        ...
        // 结束性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('ultra_optimized_sync_data');
            $processing_time = microtime(true) - $start_time;
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('sync_data_query_time', $processing_time);
        }
    ...
    public static function ultra_batch_update_sync_status(array $sync_updates): bool {
        ...
        // 开始性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('ultra_batch_sync_update');
        }
        ...
        try {
            // 使用优化的批量更新方法
            $success = \NTWP\Infrastructure\Database\DatabaseHelper::batch_update_sync_status($sync_updates);
        ...
        // 结束性能监控
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('ultra_batch_sync_update');
            $processing_time = microtime(true) - $start_time;
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('sync_status_update_time', $processing_time);
        }
    ...
    public static function api_filtered_incremental_sync(string $database_id, $notion_api, array $sync_options = []): array {
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('api_filtered_sync');
        }
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('api_filtered_sync');
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('api_filtered_sync_time', $stats['processing_time']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('api_filtered_pages', $stats['filtered_pages']);
            if (isset($stats['bandwidth_saved'])) {
                \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('api_bandwidth_saved', $stats['bandwidth_saved']);
            }
        }
    ...
    public static function batch_api_filtered_sync(array $database_configs, $notion_api): array {
        ...
        // 记录批量同步统计
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_api_sync_time', $batch_stats['total_processing_time']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_api_sync_databases', $batch_stats['total_databases']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_api_sync_pages', $batch_stats['total_pages_filtered']);
        }
    ...
    public static function concurrent_api_sync(string $database_id, $notion_api, array $sync_options = []): array {
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::start_timer('concurrent_api_sync');
        }
        ...
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::end_timer('concurrent_api_sync');
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('concurrent_api_sync_time', $stats['processing_time']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('concurrent_api_pages', $stats['concurrent_pages']);
            if (isset($stats['performance_gain'])) {
                \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('concurrent_api_gain', $stats['performance_gain']);
            }
        }
    ...
    public static function batch_concurrent_api_sync(array $database_configs, $notion_api): array {
        ...
        // 记录批量并发同步统计
        if (class_exists('\\NTWP\\Core\\Performance\\PerformanceMonitor')) {
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_concurrent_sync_time', $batch_stats['total_processing_time']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_concurrent_databases', $batch_stats['total_databases']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_concurrent_pages', $batch_stats['total_pages_concurrent']);
            \NTWP\Core\Foundation\Performance\PerformanceMonitor::record_custom_metric('batch_concurrent_gain', $batch_stats['total_performance_gain']);
        }
