        ...
        foreach ($pages as $pageData) {
            try {
                ...
                if (class_exists('Notion_Import_Coordinator')) {
                    $result = Notion_Import_Coordinator::import_single_page($pageData, $config);
                    if ($result) {
                        $successCount++;
                    } else {
                        $errorCount++;
    ...
    private function executeUpdatePages(array $pages, array $config): array {
        ...
        foreach ($pages as $pageData) {
            try {
                // 使用现有的更新逻辑
                if (class_exists('Notion_Import_Coordinator')) {
                    $result = Notion_Import_Coordinator::update_single_page($pageData, $config);
                    if ($result) {
                        $successCount++;
                    } else {
                        $errorCount++;
        ...
        foreach ($images as $imageData) {
            try {
                ...
                if (class_exists('Notion_Image_Processor')) {
                    $result = Notion_Image_Processor::process_image($imageData, $config);
                    if ($result) {
                        $successCount++;
                    } else {
                        $errorCount++;
    ...
    private function executeSyncIncremental(array $data, array $config): array {
        try {
            if (class_exists('Notion_Incremental_Detector')) {
                return Notion_Incremental_Detector::process_incremental_sync($data, $config);
            } else {
                // 基础增量同步逻辑
