...
class TaskExecutor {
    ...
    private function executeImportPages(array $pages, array $config): array {
        ...
        foreach ($pages as $pageData) {
            try {
                // 使用现有的导入逻辑
                if (class_exists('NTWP\Handlers\ImportHandler')) {
                    $import_handler = new \NTWP\Handlers\ImportHandler(new \NTWP\Services\Import\ImportService());
                    $result = $import_handler->import_notion_page($pageData);
                    if ($result) {
                        $successCount++;
                    } else {
                        $errorCount++;
        ...
        foreach ($images as $imageData) {
            try {
                ...
                if (class_exists('NTWP\Services\Image_Processor')) {
                    $result = \NTWP\Services\Content\ImageProcessor::process_image($imageData, $config);
                    if ($result) {
                        $successCount++;
                    } else {
                        $errorCount++;
    ...
    private function executeSyncIncremental(array $data, array $config): array {
        try {
            if (class_exists('NTWP\Services\Incremental_Detector')) {
                return \NTWP\Services\Sync\IncrementalDetector::process_incremental_sync($data, $config);
            } else {
                // 基础增量同步逻辑
