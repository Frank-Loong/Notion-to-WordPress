...
class Container {
    ...
    public static function get(string $name): mixed {
        ...
        try {
            ...
            if (class_exists('NTWP\Core\Foundation\Logger')) {
                \NTWP\Core\Foundation\Logger::error_log($error_message, 'Dependency Container');
            }
    ...
    public static function init_core_services(): void {
        ...
        self::register('network', function() {
            return new \NTWP\Infrastructure\Concurrent_Network_Manager();
        });
        
        // 注册任务调度服务（单例模式）
        self::register('scheduler', function() {
            return \NTWP\Core\Foundation\Task\AsyncTaskScheduler::getInstance();
        });
        
        // 注册API合并服务
        self::register('api_merger', function() {
            return new \NTWP\Infrastructure\Smart_API_Merger();
        });
        ...
        self::register('unified_concurrency_adapter', function() {
            return new \NTWP\Infrastructure\Unified_Concurrency_Manager(); // 使用适配器类
        });
        ...
        self::register('content_converter_adapter', function() {
            return new \NTWP\Services\Content\ContentConverter_Adapter();
        });
    }
