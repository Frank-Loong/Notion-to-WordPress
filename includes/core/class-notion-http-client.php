...
class Notion_HTTP_Client {
    ...
    public static function safe_remote_get_with_retry(string $url, array $args = [], int $max_retries = 3) {
        ...
        if (class_exists('Notion_Network_Retry')) {
            return Notion_Network_Retry::execute_with_retry(function() use ($url, $args) {
    ...
    private static function process_response($response, string $url) {
        // 检查错误
        if (is_wp_error($response)) {
            Notion_Logger::error_log(__('远程请求失败: ', 'notion-to-wordpress') . $response->get_error_message(), 'HTTP');
            return $response;
        }
        ...
        if ($status_code >= 400) {
            ...
            Notion_Logger::error_log($error_msg, 'HTTP');
