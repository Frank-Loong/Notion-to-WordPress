...
class Notion_Async_Task_Scheduler {
    ...
    private function scheduleWithModernEngine(string $task_type, array $task_data, array $options): string {
        ...
        switch ($task_type) {
            case self::TASK_INCREMENTAL_SYNC:
                ...
                $config['priority'] = Notion_Modern_Async_Engine::PRIORITY_HIGH;
                break;
                
            case self::TASK_BATCH_IMPORT:
                $config['batch_size'] = $options['batch_size'] ?? 20;
                $config['priority'] = Notion_Modern_Async_Engine::PRIORITY_NORMAL;
                break;
                
            case self::TASK_IMAGE_PROCESSING:
                $config['batch_size'] = $options['batch_size'] ?? 10;
                $config['priority'] = Notion_Modern_Async_Engine::PRIORITY_LOW;
                break;
        ...
        return Notion_Modern_Async_Engine::execute($task_type, $task_data, $config);
    }
    ...
    public function get_task_status(string $task_id): array {
        // 如果使用现代异步引擎，委托给它
        if (class_exists('Notion_Modern_Async_Engine')) {
            return Notion_Modern_Async_Engine::getProgress($task_id);
        }
