...
class Notion_To_WordPress_Lock {
    ...
    public function is_valid(): bool {
        ...
        // 检查是否是当前持有者
        if ($lock_data->option_value !== $this->lock_holder) {
            $this->is_locked = false;
            return false;
        }

        // 检查是否过期（autoload字段存储过期时间戳）
        $expire_time = (int) $lock_data->autoload;
